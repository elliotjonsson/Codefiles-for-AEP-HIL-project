#!/usr/bin/env python3
# encoding: utf-8
# 修改版：将机器人转向从右摇杆X轴改为左摇杆X轴，释放右摇杆用于云台控制
import os
import math
import rclpy
...
class JoystickController(Node):
    def __init__(self, name):
        ...
        self.get_logger().info('\033[1;32m%s\033[0m' % '修改版启动: 左摇杆X控制转向, 右摇杆释放给云台')
    ...
    def axes_callback(self, axes):
        ...
        if self.machine == 'MentorPi_Mecanum':
            ...
            # 注意：取消了左摇杆X的左右平移功能，如需要可以映射到其他轴
            twist.linear.y = 0  # 禁用侧向移动
            ...
            # 左摇杆Y轴：前后移动
            twist.linear.x = val_map(axes['ly'], -1, 1, -self.max_linear, self.max_linear)
            # 左摇杆X轴：转向角度控制（原来是右摇杆X）
            steering_angle = val_map(axes['lx'], -1, 1, -math.radians(45), math.radians(45))
